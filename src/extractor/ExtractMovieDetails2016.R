...
mainPage <- nominees
mainPage$MovieURL <- paste0("http://www.imdb.com/title/",mainPage$Webpage)
...
extractSingleMovieDetail <- function(url, pause){
  ...
  MovieTitle <- source.page %>%
    ...
    html_text()
  MovieTitle <- ifelse(length(MovieTitle)<1,NA,MovieTitle[1])
  ...
  #NumOscarWins- corrected, ensure it is only pulling Oscars won
...
#   MovieNumUserRatings <- ifelse(length(MovieNumUserRatings)<1,NA,as.integer(MovieNumUserRatings))
  ...
  returnList <- list(MovieTitle, url, release_yr, movie_rating, MovieNumUserRatings, MovieNumCriticRatings,
                     duration_mins, aspectRatio, MPAArating,
                     country, language, ReleaseDate, genre1, genre2, genre3, #numImdbCriticReviews,
                    directorName, directorID, directorURL, star1Name, star1Id, star1URL,
                    star2Name, star2Id, star2URL, star3Name, star3Id, star3URL, 
                    budget, budgetCurr, openweekend, openweekendCurr, gross, grossCurr, num)  
  names(returnList) = c("MovieTitle","MovieURL","ReleaseYear", "IMDBRating","NumberOfUsersRated","NumberOfCriticsRated",
                        "DurationMins","AspectRatio",
