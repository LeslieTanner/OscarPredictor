...
load(file=paste0(dataDir, "movieDFClean.RData"))
...
########
# Look at Release Year of Movies
png(filename = "~/STAT6021/OscarProject/OscarPredictor/src/visual/Barplot-ReleaseYear.png",
    width = 5, height = 3, units = 'in', res = 300)
barplot(table(movieDataFrame$ReleaseYear), ylim = c(0, 400), main = "Movie Release Year", 
...
movieDataFrame$DurationMins[which(movieDataFrame$DurationMins > 480)]  # One movie is 2880 minutes long
movieDataFrame$MovieTitle[which(movieDataFrame$DurationMins > 600)]
########
# Throw out "The Longest Most Meaningless Movie in the World"
